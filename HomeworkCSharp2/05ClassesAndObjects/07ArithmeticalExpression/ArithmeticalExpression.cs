...
public class ArithmeticalExpression
{
    ...
    public static int Priority(string arithmeticOperator)
    {
    ...
    public static Queue<string> ConvertToReversePolishNotation(List<string> tokens)
    {
        ...
        for (int i = 0; i < tokens.Count; i++)
        {
            ...
            {
                while (stack.Count != 0 && arithmeticOperations.Contains(stack.Peek()[0]) && Priority(currentToken) <= Priority(stack.Peek()))
                {
                    queue.Enqueue(stack.Pop());
                }
